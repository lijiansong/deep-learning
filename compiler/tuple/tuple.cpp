...
template <size_t N, typename... T> using choose = type_of<Choose<N, T...>>;
...
template <size_t L> using range = type_of<Range<L>>;

// single tuple element
template <size_t N, typename T> class TupleElem {
  T elem;
...
public:
  template <size_t M> pick<M> &get() { return elem<M>::get(); }

  template <size_t M> const pick<M> &get() const { return elem<M>::get(); }
};

template <typename... T> struct Tuple : TupleImpl<range<sizeof...(T)>, T...> {
...
int main() {
  ...
  Tuple<float, int, int, int, int, int, int, int, int, int, int, int, int, int,
        int, int, int>
      test;
  Tuple<> test2;
